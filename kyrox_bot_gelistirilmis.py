...
async def on_presence_update(before, after):
    if not after.bot:
        rank_online[(after.guild.id, after.id)] = (after.status == discord.Status.online)
...
async def warn(interaction: discord.Interaction, member: discord.Member, reason: str = "Sebep yok"):
    ...
    data["warnings"].setdefault(gid, {}).setdefault(uid, []).append({"reason": reason, "time": datetime.utcnow().isoformat()})
    save_data()
