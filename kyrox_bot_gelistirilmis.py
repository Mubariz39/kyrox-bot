...
# ONLINE / OFFLINE TAKİBİ İÇİN GEREKLİ
...
# Artık rank_activity kullanılmıyor.
# Rank tamamen Discord ONLINE durumuna göre ilerliyor.
...
async def set_rank_role(
    ...
    # Botun rol sırası kontrolü
    if target_role >= me.top_role:

        print(
            f"[RANK] {guild.name}: "
            f"{target_role.name} verilemedi."
        )

        print(
            "[RANK] Botun rolünü rank rollerinin "
            "ÜZERİNE taşı."
        )

        return

    # Bütün rank rol isimlerini al
    rank_role_names = {

        name

        for name, _ in RANK_ROLES.values()
    ...
    try:
        ...
        print(
            "[RANK] Botta 'Rolleri Yönet' yetkisi "
            "olduğundan emin ol."
        )
...
# =========================================================
# MEVCUT RANKA GÖRE ROLÜ SENKRONİZE ET
# =========================================================
...
# =========================================================
# TÜM SUNUCUNUN RANKLARINI SENKRONİZE ET
# =========================================================

async def sync_all_rank_roles(
    guild
):
...
async def on_ready():
    ...
    try:
        ...
        if TEST_GUILD_ID:
            ...
            print(
                "TEST SUNUCUSU slash komutları "
                f"senkronize edildi: {len(synced)}"
            )
    ...
    for guild in bot.guilds:
        ...
        try:
            ...
            for member in guild.members:
                ...
                # Mevcut rankına uygun rolü tekrar ver
                await sync_member_rank_role(
                    member
                )
...
async def on_presence_update(
    ...
    # Sadece ONLINE sayılır
    if after.status == discord.Status.online:

        rank_online[key] = True

    else:

        rank_online[key] = False
...
async def mute(
    ...
    if member.top_role >= interaction.user.top_role:
        ...
        return await interaction.response.send_message(
            ...
            "❌ Bu kullanıcı seninle aynı "
            "veya daha yüksek role sahip.",
...
@bot.tree.command(
    name="rank",
    description="Online sürene göre rank bilgini gösterir."
)
@app_commands.describe(
...
async def rank_list(
    ...
    for i, (
        ...
        if rank > 0:
            ...
            rank_name = RANK_ROLES[
                rank
            ][0]

        else:

            rank_name = "Henüz Rank Yok"
    ...
    if target is None:
        ...
        embed.add_field(
            ...
            value=(
                ...
                f"⏳ Kalan: "
                f"**{format_duration(target - seconds)}**"
...
async def on_message(
    ...
    if len(
        ...
        try:
            ...
            await message.channel.send(
                ...
                f"🚨 {message.author.mention} "
                f"spam nedeniyle 30 saniye susturuldu.",
    ...
    # -----------------------------------------------------
    # ÖNEMLİ:
    # Burada artık rank süresi EKLENMİYOR.
    #
    # Rank sistemi Discord ONLINE durumundan
    # rank_tracker_loop() tarafından hesaplanıyor.
    # -----------------------------------------------------
...
async def on_member_join(
    ...
    # Yeni üyenin online durumunu kaydet
    rank_online[
        (
            member.guild.id,
            member.id
    ...
    while not bot.is_closed():
        ...
        for guild in bot.guilds:
            ...
            for member in guild.members:
                ...
                # -------------------------------------------------
                # GERÇEK DISCORD DURUMUNU KONTROL ET
                # -------------------------------------------------
                ...
                rank_online[key] = is_online

                # -------------------------------------------------
                # OFFLINE / IDLE / DND İSE SÜRE EKLEME
                # -------------------------------------------------
